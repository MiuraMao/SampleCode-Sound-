...
void FFT(double x_real[], double x_imag[], int N)
{
    ...
    {
        for (i = 0; i < pow2(stage - 1); i++)   //2のべき乗
        {
            for (j = 0; j < pow2(number_of_stage - stage); j++)
            {
                ...
                if (stage < number_of_stage)
                {
                    ...
                    x_real[m] = (a_real - b_real) * c_real - 
                                (a_imag - b_imag) * c_imag;
                    x_imag[m] = (a_imag - b_imag) * c_real +
                                (a_real - b_real) * c_imag;
                }
                else
                {
                    x_real[n] = a_real + b_real;
